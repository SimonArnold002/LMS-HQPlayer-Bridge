...
HQPlayer has no decoder for **ALAC, AAC or anything else in an MP4/M4A container**, so those don't go over as files — LMS converts them to FLAC and the plugin serves them itself, on the same path it uses for streaming services. You don't have to do anything; the plugin checks the format and picks the route.

### Streaming services and radio

Qobuz, Tidal, Deezer, internet radio and everything else remote is streamed through LMS, which transcodes as needed (FLAC first, so usually there's no transcoding at all — a 24/96 Qobuz track reaches HQPlayer at full rate). The service plugin handles its own authentication as usual — the HQPlayer player looks like any other player to it.

These have no file to hand over, so the plugin serves the audio itself on a plain path of its own — the same route anything converted takes. That is not a detail you have to care about, with one consequence you might notice: see the note on gapless below.
...
- **Gapless works on streaming too, though not by the same mechanism.** Local files HQPlayer reads natively are handed over early, so the transition is seamless by design. Streaming services and converted files can't be — that audio passes through LMS and a player has only one such stream at a time — so the next track is loaded when the current one ends. That load takes about half a second, and HQPlayer still has around a second and a half of audio in flight to your endpoint at that moment, so nothing is heard. A **sample rate change** between tracks is the exception: HQPlayer has to retune its output, which takes a couple of seconds and is audible. Within an album that's rarely an issue.
...
- **Every local format plays, including ones HQPlayer can't decode.** HQPlayer has no decoder for ALAC, AAC or m4a, so LMS converts those to FLAC on the way out and the plugin serves them itself. They play and seek, and in practice transition seamlessly for the reason described under gapless below. Formats HQPlayer decodes natively (FLAC, WAV, AIFF, DSD, WavPack, MP3, Ogg) are passed through untouched, byte for byte, and stay gapless.
