...
- **Streaming services don't play yet.** Qobuz, Tidal, Deezer and radio are not working. The cause is now known and it's a narrow one: **HQPlayer won't fetch a URL that contains a `?`**, and the URL LMS serves a remote track on is `/stream.mp3?player=…`. HQPlayer accepts the track, answers OK, and then simply never requests it. The fix is an endpoint the plugin serves on a plain path, which is the next thing being built. Your local library is unaffected.
