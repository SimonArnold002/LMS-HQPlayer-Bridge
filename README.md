...
- **Gapless on local files, not on streaming services.** The next track is handed to HQPlayer's own playlist while the current one is still playing, so HQPlayer makes the transition itself with no gap. That needs each track to have its own URL, which is true of your local library but not of Qobuz, Tidal or radio — those all come through one LMS stream URL, so a streaming track is loaded when the previous one ends and there's a brief gap before it.
