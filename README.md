...
These are the one case with no file to hand over, so the plugin serves the audio itself on a plain path of its own. That is not a detail you have to care about, with one consequence you might notice: see the note on gapless below.
...
- **Every local format plays, including ones HQPlayer can't decode.** HQPlayer has no decoder for ALAC, AAC or m4a. Those are asked for as FLAC so LMS converts them on the way out, which means they play, and stay gapless — but they can't be seeked, because a converted stream has no known length for LMS to seek within. Formats HQPlayer decodes natively (FLAC, WAV, AIFF, DSD, WavPack, MP3, Ogg) are passed through untouched and seek normally.
