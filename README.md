...
- **Streaming services play, but without gapless.** Qobuz, Tidal, Deezer and radio are served on a path of the plugin's own, because HQPlayer won't fetch a URL containing a `?` and the URL LMS gives a remote track is `/stream.mp3?player=…`. Unlike a local file, that audio passes through LMS, and a player has only one such stream at a time — so the next track can't be handed over early, and there's a short gap between streaming tracks. Local files in a format HQPlayer reads natively are unaffected and stay gapless.
...
- **Every local format plays, including ones HQPlayer can't decode.** HQPlayer has no decoder for ALAC, AAC or m4a, so LMS converts those to FLAC on the way out and the plugin serves them itself. They play and seek, but not gaplessly — converted audio goes through the same single stream the streaming services use. Formats HQPlayer decodes natively (FLAC, WAV, AIFF, DSD, WavPack, MP3, Ogg) are passed through untouched, byte for byte, and stay gapless.
