...
| **ReplayGain everywhere** | Library *and* streaming tracks are normalised in HQPlayer using the figure LMS worked out, adjusted for HQPlayer's own headroom | Replay gain set to anything but Off in LMS |
...
- **The gain is adjusted for HQPlayer's convolution gain compensation, and never pushed into clipping.** If you have convolution enabled with gain compensation set, HQPlayer applies that on top of anything the plugin sends — so sending a ReplayGain figure raw would quietly attenuate every track by the headroom as well. The plugin reads that compensation from HQPlayer and adds it back, so tracks land on their ReplayGain target rather than below it, and it re-reads it as you play so a change in HQPlayer takes effect within a track. Where a track calls for a *boost* — quietly mastered albums often do — the boost is allowed only as far as that compensation permits, and trimmed rather than refused beyond that. **A track with no ReplayGain data at all is left completely alone**, rather than being lifted into the headroom. If you have replay gain switched **off** in LMS, the plugin sends nothing at all for library tracks and HQPlayer's own tag reading is left alone.
