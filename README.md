...
- **The gain is adjusted for HQPlayer's own headroom, and never pushed into clipping.** HQPlayer holds back a few dB for its DSP, and it applies that on top of anything the plugin sends — so sending a ReplayGain figure raw would quietly attenuate every track by the headroom as well. The plugin reads the headroom from HQPlayer and adds it back, so tracks land on their ReplayGain target rather than below it. Where a track calls for a *boost* — quietly mastered albums often do — the boost is allowed only as far as the headroom permits, and trimmed rather than refused beyond that. **A track with no ReplayGain data at all is left completely alone**, rather than being lifted into the headroom. If you have replay gain switched **off** in LMS, the plugin sends nothing at all for library tracks and HQPlayer's own tag reading is left alone.
